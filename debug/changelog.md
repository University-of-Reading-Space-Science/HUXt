...
- In HUXt v4 we introduced the `cme_fixed_duration` and `fixed_duration` keywork arguments for initialising a ConeCME class. At this time `cme_fixed_duration` was set to False by default, and `thickness` was set to 5Rs by default. Research since has shown that 1) the CME fixed duration geometry gives more physical simulations and 2) that when using the CME fixed duration geometry, the thickness parameter should be set to 0Rs. We've made these the default settings in V5.0.0, with the fixed duaration defaulting to our best estimate of what it should be on average; this was computed by minimising the mean arrival time error for a set of Earth directed CMEs. 
