...
class HUXt2DCME:
    ...
    def __init__(self, simtime=5.0, dt_scale=1.0):
        ...
        r_max = 240.0
        self.Nr = 140
        self.r, self.dr = np.linspace(r_min, r_max, self.Nr, retstep=True)
        ...
        self.longRef = self.twopi * (
                    1.0 - 11.0 / 27.0)  # this sets the Carrington longitude of phi=180. So if this is the E-S line,
    ...
    def solve1d(self, v_boundary):
        """
        Functon to solve Burgers equation for the time evolution of the radial wind speed, given a variable input
        boundary condition.
        """
...
def _upwind_step_(model, v_up, v_dn):
    ...
    the solar wind.
    Model should be an instance of a HUXt model class, as it must contain the models domain and parameters.
