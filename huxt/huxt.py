"""
These classes and functions contain the core functionality of the HUXt numerical model. They are
used to set up HUXt, run HUXt, and save and load the results.
"""
import copy
import errno
import os
...
class Observer:
    """
    A class returning the HEEQ and Carrington coordinates of a specified Planet or spacecraft,
    for a given set of times. Planets include the inner planets out to Saturn. The ephemeris data
    for Jupiter and Saturn correspond to the Jupiter system barycenter and Saturn system
    barycenter. Spacecraft include ACE, STEREO-A, STEREO-B, Parker Solar Probe, and Solar Orbiter.
    The ephemeris data are downloaded from JPL Horizons. Planetary positions are linearly
    interpolated from a 12-hour resolution ephemeris that spans 1974-01-01 until 2029-01-01.
    Spacecraft positions are linearly interpolated from a 3-hour resolution ephemeris that spans
    the available duration on JPL Horizons for each mission. JPL Horizons only provides ACE and
    STEREO-A data for short windows into the future (~70 and ~100 days, respectively). And so the
    ephemeris file may need to be periodically updated. The ephemeris data can be updated using
    the huxt/scripts/generate_huxt_ephemeris.py script.

    Attributes:
        body: String name of the planet or spacecraft.
        lat: HEEQ latitude of the body at all values of time.
        lat_c: Carrington latitude of the body at all values of time.
        ...
        r_c: Carrington radius of the body at all values of time.
        r_hae: HAE radius of the body at all values of time.
    ...
    def __init__(self, body, times):
        ...
        # Get the path to the ephemeris file and open
        dirs = _setup_dirs_()
        ephem = h5py.File(dirs['ephemeris'], 'r')

        # Now get observer's coordinates
        all_time = Time(ephem[self.body]['HEEQ']['time'], format='jd')

        # STEREO-A and ACE have shorter lengths of ephemeris data. Check requested times are OK.
        if np.any(times > all_time[-1]):
            raise ValueError(f"{body} ephemeris extends to {all_time[-1].isot}. Requested times"
                             f"are outside this limit. Updating the HUXt ephemeris file may "
...
class ConeCME:
    ...
    Attributes:
        ...
        initial_height: Initiation height of the CME, in km. Defaults to HUXt inner boundary at
                       30 solar radii.
        radius: Initial radius of the CME, in km.
        thickness: Thickness of the CME cone, in km.
        coords: Dictionary containing the radial and longitudinal (for HUXT2D) coordinates of the
                of Cone CME for each model time step.
    """

    # Some decorators for checking the units of input arguments
    @u.quantity_input(t_launch=seconds)
    @u.quantity_input(longitude=deg)
    @u.quantity_input(latitude=deg)
    @u.quantity_input(v=km_per_s)
    @u.quantity_input(width=deg)
    @u.quantity_input(thickness=solRad)
    @u.quantity_input(fixed_duration=seconds)
    def __init__(self, t_launch=0.0 * seconds, longitude=0.0 * deg, latitude=0.0 * deg,
        ...
        self.initial_height = initial_height  # (should match the inner boundary of HUXt)
        self.radius = self.initial_height * np.tan(self.width / 2.0)  # Initial radius of CME
        self.thickness = thickness  # Extra CME thickness
        self.coords = {}
        self.frame = 'NA'
        self.longitude_huxt = -1 * rad  # the HUXt longitude, adjusted for sidereal if necessary
    ...
    def _track_(self, model, cme_id):
        """
        Tracks the perimeter of each ConeCME through the solution in the HUXt model instance.
        Updates the ConeCME.coords dictionary of CME coordinates.
        Args:
            model: A HUXt instance with a solution containing ConeCMEs
            cme_id: ID number of the CME to link the ConeCME object with the CME tracer fields.
        Returns:
             None
        """
    ...
    def compute_arrival_at_body(self, body_name):
        """
        Compute the arrival of the CME at a solar system body. Available bodies are those
        accepted by the observer class, Mercury, Venus, Earth, STA, and STB. Takes account of
        differences between synodic and sidereal frames
        Args:
            body_name: String body name as accepted by the Observer class, including Mercury, Venus,
             Earth, STA, and STB.
        Returns:
             arrival_stats: A dictionary of the arrival stats of the CME, with keys hit, hit_id,
                            t_arrive, t_transit, lon, r, and v.
        """
    ...
    def compute_arrival_at_location(self, longitude, radius):
        """
        Compute the arrival of the CME at a location specified with a longitude and radius. Takes
        account of differences between synodic and sidereal frames.
        Args:
            longitude: location longitude at t=0 of the model run. It should be in radians and be
                       a scalar or have the same size as time.
            radius: location radius at t=0 of the model run.It should be in radians and be
                       a scalar or have the same size as time.
        Returns:
             arrival_stats: A dictionary of the arrival statistics of the CME, with keys hit,
                            hit_id, t_arrive, t_transit, lon, r, and v.
        """
        ...
        if not is_scalar:
            ...
            if not match_len:
                raise ValueError('longitude and radius must be single values or be arrays of '
        ...
        for i, coord in self.coords.items():
            ...
            if np.any(front_id):
                ...
                if len(lon_cme) > 1:
                    # Look up the cme front radial coord along body longitude
                    ...
                    # If body and cme are within 1.5 deg of each other, assume they hit.
...
class HUXt:
    """
    A class containing the HUXt model described in Owens et al. (2020,
    ...
    Attributes:
        ...
        cr_lon_init: The initial Carrington longitude of Earth at the models initial timestep
                     (2 pi at the start of the CR, 0 at the end).
        ...
        dt_scale: Integer scaling number to set the model output time step relative to the models
                  CFL time step.
        ...
        time_init: The UTC time corresponding to the initial Carrington rotation number and
                   longitude. Else, NaN.
        ...
        v_grid: Array of model speed including ConeCMEs for each time, radius, and longitude,
                in km/s.
    """

    # Decorators to check units on input arguments
    @u.quantity_input(v_boundary=km_per_s)
    @u.quantity_input(simtime=day)
    @u.quantity_input(cr_lon_init=deg)
    def __init__(self, v_boundary=np.nan * km_per_s, b_boundary=np.nan, cr_num=np.nan,
                 cr_lon_init=360.0 * deg, latitude=0 * deg, r_min=30 * solRad,
        ...
        Initialise the HUXt model instance.
            ...
            lon_out: A specific single longitude (relative to Earth) to compute HUXt solution along,
                     in degrees
            ...
            input_v_ts: Time series of inner boundary V conditions. For initialising HUXt with,
                        for example, in-situ observations from L1. If used as a keyword argument,
                        this overrides v_boundary input.
            input_bv_ts: Time series of inner boundary B conditions. For initialising HUXt with,
                         for example, in-situ observations from L1. If used as a keyword argument,
                         overrides b_boundary input.
            ...
            input_iscme_ts: Boolean mask time series indicating what time steps correspond to CMEs
                            in input_v_ts. If used as a keyword argument, overrides ConeCMEs past to
                            huxt.sovle().
            save_full_v: Boolean flag to determine if the full v field (including spin up) is
        ...
        # Set the frame of reference. Synodic keeps ES line at 0 degrees longitude.
        ...
        if (self.frame == 'sidereal') & (self.nlon == 1):
            print("Warning: HUXt configured for a 1-D run in the sidereal frame. This simulation "
                  "will not work correctly with functions like "
                  "huxt_analysis.get_observer_time_series()")
        ...
        if np.all(np.isnan(v_boundary)):
            ...
            lon_boundary, dlon, _ = longitude_grid()
            ...
            nv = len(v_boundary)
            dlon = 2 * np.pi / nv
            self.v_boundary_lons = np.arange(dlon / 2, 2 * np.pi - dlon / 2 + dlon / 10, dlon)*rad
        ...
        if np.all(np.isnan(b_boundary)):
            ...
            nb = len(b_boundary)
            dlon = 2 * np.pi / nb
            self.b_boundary_lons = np.arange(dlon / 2, 2 * np.pi - dlon / 2 + dlon / 10, dlon)*rad

        # add a flag for tracking streaklines
        self.track_streak = False

        # Determine the CR number used for spacecraft/planetary positions.
        if np.isnan(cr_num):
        ...
        # Check cr_lon_init, make sure in 0-2 pi range.
        self.cr_lon_init = cr_lon_init.to('rad')
        if (self.cr_lon_init < 0.0 * rad) | (self.cr_lon_init > self.twopi * rad):
            print(f"Warning: cr_lon_init={self.cr_lon_init}, outside expected range. "
                  f"Rectifying to 0-2pi.")
        ...
        self.v_boundary = np.interp(self.v_boundary_lons.value, lon_shifted, v_b_shifted,
                                    period=self.twopi)
        ...
        self.b_boundary = np.interp(self.b_boundary_lons.value, lon_shifted, b_b_shifted,
                                    period=self.twopi)

        # Compute the buffertime required to spin up HUXt, based on minimum speed on the inner
        # boundary and span of radial grid
        self.buffertime = 1.5 * (self.rrel[-1] / self.v_boundary.min()).to(day)

        # Preallocate space for the output for the solar wind fields
        ...
        if not np.all(np.isnan(input_v_ts)):
            ...
            full_lon_grid, dlon, _ = longitude_grid()
            _, _, y_ind = np.intersect1d(self.lon.value, full_lon_grid.value, return_indices=True)
    ...
    def ts_from_vlong(self):
        ...
        # How many radians of Carrington rotation are in the spin-up period?
        ...
        for i in range(self.lon.size):
            ...
            vinit = np.interp(loninit, self.v_boundary_lons.value, self.v_boundary.value,
                              period=2 * np.pi)
            ...
            if self.track_b:
                binit = np.interp(loninit, self.b_boundary_lons.value, self.b_boundary,
                                  period=2 * np.pi)
                # convert from cr longitude to timesolve
    ...
    @u.quantity_input(streak_carr=rad)
    def solve(self, cme_list, streak_carr=np.array([])*rad, save=False, tag=''):
        """
        Solve HUXt for the provided longitudinal boundary conditions and cme list. Updates the
        HUXt.v_grid
        Args:
            cme_list: A list of ConeCME instances to use in solving HUXt
            streak_carr: An numpy array of Carrington longitudes from which to trace streaklines,
                         units of radians.
            save: Boolean, if True saves model output to HDF5 file
            tag: String, appended to the filename of the saved solution.
        Returns:
            None
        """
        ...
        # Only ConeCMEs in the list
        # Make sidereal correction if necessary
        # Check launch time is not in the spin up period before simulation time.
        cme_list_checked = []
        for cme in input_cme_list:
            ...
            if isinstance(cme, ConeCME):
                ...
                if self.frame == 'sidereal':
                    ...
                    cme.longitude_huxt = zerototwopi(cme.longitude + cme_hae) * rad
                else:
                ...
                if cme.t_launch >= 0*seconds:
                    ...
                    print(f"Warning: ConeCME had negative t_launch ({cme.t_launch}), which is not"
                          f" allowed.")
                    print("Warning: This ConeCME object was not passed into the HUXt solver")
            else:
                print("Warning: cme_list contained objects other than ConeCME instances.  These "
                      "were excluded")

        self.cmes = cme_list_checked

        # If CMEs parsed, get an array of their parameters for using with the solver
        if len(self.cmes) > 0:
        ...
        # check CME speeds aren't so fast they will butt up against the CFL condition.
        if len(self.cmes) > 0:
            constants = huxt_constants()
            ...
            for cme in self.cmes:
                ...
                if cme.v >= v_max:
                    raise ValueError(f'CME speed {cme.v} is larger than allowed for CFL limit  of'
                                     f' {v_max}')
                if cme.v >= 0.8 * v_max:
                    print(f'Warning: CME speed of {cme.v} is close to CFL limit of {v_max}. '
        ...
        if self.input_iscme_ts_flag:
            ...
            # Create a dummy CME list to sort the boundaries
            self.cmes = []
            for _ in range(0, n_cme):
        ...
        if self.track_b:
            ...
            self.hcs_particles_r = (np.zeros((n_hcs_max, self.nt_out, 2, self.nlon))
        ...
        if isinstance(streak_carr, u.Quantity) & (streak_carr.size > 0):
            ...
            # work out the source longitude at the start of spin up
            for istreak, carr in enumerate(streak_carr):
                ...
                for ilon, lon in enumerate(self.lon):
                    for irot in range(0, nrot):
                        ...
                        if 0 < id_in < len(time_from_start) - 1:
        ...
        # ======================================================================
        for i in range(self.lon.size):
            i, v, cme_r_bounds, cme_v_bounds, hcs_r, streak_r = self.process_longitude(i,
                                                                                       n_cme,
                                                                                       n_hcs_max,
                                                                                       streak_times)
    ...
    def save(self, tag=''):
        ...
        filename = f"HUXt_CR{np.int32(self.cr_num.value):03d}_{tag}.hdf5"
        ...
        # Loop over the attributes of the model instance and save select keys/attributes.
    ...
    def get_observer(self, body):
        """
        Returns an instance of the Observer class, giving the HEEQ and Carrington coordinates at
        each model timestep. This is only well-defined if the model was initialised with a
        Carrington rotation number.
        Args:
            body: String specifying which body to look up.
        Returns:
...
class HUXt3d:
    """
    A class containing a list of HUXt classes, to enable multiple latitudes to be simulated,
    plotted, animated, etc. together
    ...
    # Decorators to check units on input arguments
    @u.quantity_input(v_map=km_per_s)
    @u.quantity_input(v_map_lat=rad)
    @u.quantity_input(v_map_long=rad)
    @u.quantity_input(latitude_max=deg)
    @u.quantity_input(latitude_min=deg)
    @u.quantity_input(simtime=day)
    @u.quantity_input(cr_lon_init=deg)
    def __init__(self, v_map=np.nan * km_per_s, v_map_lat=np.nan * rad,
        ...
        longs, _, _ = longitude_grid(lon_start=0.0 * rad, lon_stop=2 * np.pi * rad)
...
@u.quantity_input(r_min=solRad)
@u.quantity_input(r_max=solRad)
def radial_grid(r_min=30.0 * solRad, r_max=240. * solRad):
    """
    Define the radial grid of the HUXt model. Step size is fixed, but inner and outer boundary
    may be specified.
    Args:
        r_min: The heliocentric distance of the inner radial boundary.
        r_max: The heliocentric distance of the outer radial boundary.
    Returns:
        r: An array of radial coordinates, relative to the Sun's center, in solar radii.
...
@u.quantity_input(lon_out=rad)
@u.quantity_input(lon_start=rad)
@u.quantity_input(lon_stop=rad)
def longitude_grid(lon_out=np.nan * rad, lon_start=np.nan * rad, lon_stop=np.nan * rad):
    ...
    if single_longitude:
        # Lon_out takes precedence over lon_min and lon_max
...
@u.quantity_input(latitude_min=rad)
@u.quantity_input(latitude_max=rad)
...
def time_grid(simtime, dt_scale):
    ...
    Returns:
        time_grid_dict: A dictionary containing arrays of the model's intrinsic time steps and the
                        requested output timesteps.
    """
    constants = huxt_constants()
    v_max = constants['v_max']
    dr = constants['dr']
...
def _setup_dirs_():
    """
    Function to pull out the directories of boundary conditions, ephemeris, and to save figures and
    output data.
    Returns:
        dirs: A dictionary of paths to HUXt directories of code, data, figures, and relevant files.
    """
...
def zerototwopi(angles):
    """
    Function to constrain angles to the 0-2 pi domain.
    Args:
        angles: a numpy array of angles.
    Returns:
        angles_out: a numpy array of angles constrained to 0-2 pi domain.
    """
...
def solve_radial(vinput, binput, iscmeinput, model_time, rrel, params,
    ...
    """
    Solve the radial profile as a function of time (including spinup), and return radial profile
    at specified output timesteps.
    ...
    Returns:
        v_grid: Array of radial solar wind speed profile as a function of time.
        cme_particles_r: Array of CME tracer particle positions as a function of time.
    ...
    for t, time in enumerate(model_time):
        ...
        # and snapshots saved to output at the right steps.
        if t == 0:
            v = np.ones(nr) * 400
        ...
        # Compute the boundary speed of each CME at this time.
        if time > 0:
            if do_cme == 1:
...
def add_cmes_to_input_series(vinput, model_time, lon, r_boundary, cme_params, latitude):
    ...
    Args:
        ...
        cme_params: Array of ConeCME parameters to include in the solution. One row for each CME,
                    with columns as required by _is_in_cone_cme_boundary_expanding_
    ...
    for t, time in enumerate(model_time):
        ...
        # Compute the boundary speed of each CME at this time.
        # Set the boundary to the maximum CME speed at this time.
        if time > 0:
...
def _upwind_step_(v_up, v_dn, dtdr, alpha, r_accel, rrel):
    ...
    # Get an estimate of the next time step
...
def _is_in_cme_boundary_(r_boundary, lon, lat, time, cme_params):
    ...
    if (y >= 0) & (y < cme_radius):
        ...
        # compute x, the distance of the current longitude from the nose
...
def load_HUXt_run(filepath):
    ...
    if os.path.isfile(filepath):
        ...
        if track_b:
            ...
            model.hcs_particles_r = (data['hcs_particles_r'][()] *
                                     u.Unit(data['hcs_particles_r'].attrs['unit']))
        if track_streak:
            model.track_streak = track_streak
            model.streak_particles_r = (data['streak_particles_r'][()] *
                                        u.Unit(data['streak_particles_r'].attrs['unit']))
        ...
        for k in all_cmes.keys():
            ...
            initial_height = (cme_data['initial_height'][()] *
                              u.Unit(cme_data['initial_height'].attrs['unit']))
            ...
            if 'cme_expansion' in cme_data:
                ...
                fixed_duration = (cme_data['fixed_duration'][()] *
                                  u.Unit(cme_data['fixed_duration'].attrs['unit']))

                cme = ConeCME(t_launch=t_launch, longitude=lon, latitude=lat, v=v, width=width,
                              thickness=thickness, initial_height=initial_height,
            ...
            for time_key, pos in coords_group.items():
                ...
                coords_data[t]['model_time'] = (pos['model_time'][()] *
                                                u.Unit(pos['model_time'].attrs['unit']))
                ...
                coords_data[t]['front_id'] = (pos['front_id'][()] *
                                              u.Unit(pos['front_id'].attrs['unit']))
...
def bgrid_from_hcs(hcs_particles_r, input_b_ts, model_time, time_out, r_grid, lons):
    ...
    for ilon in range(0, nlon):
        ...
        for t in range(0, nt):
            ...
            for ihcs, _ in enumerate(hcs_crossings_r):
...
def get_version():
    """ Returns the version of the HUXt package."""
    return "5.0.2"
