...
omni_data = HI.get_omni(starttime, endtime)
#remove ICMEs
icmes = HI.ICMElist()
omni_data = HI.remove_ICMEs(omni_data, icmes, interpolate=True, icme_buffer=0.1 * u.day, interp_buffer=1 * u.day,
...
if os.path.exists(surf_pickle_file):
    ...
    while current_time < end_time:
        ...
        # Run forecast
        try:
            is_model = HI.omniHUXt_forecast(current_time, simtime=forecast_duration,
            ...
            is_model.solve([])
            ts_surf_chunk = HA.get_observer_timeseries(is_model)
